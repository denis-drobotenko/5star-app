const { User, Client, sequelize, Op } = require('../models');
    ...
    await sequelize.transaction(async (t) => {
      ...
      if (user_type === 'CLIENT') {
        if (!client_id) {
            ...
            const error = new Error('Для пользователя типа CLIENT необходимо указать client_id');
            error.status = 400;
            throw error;
        }
        ...
        const error = new Error(`Нельзя указать client_id для пользователя типа ${user_type}`);
        error.status = 400;
        throw error;
      }
    ...
    await sequelize.transaction(async (t) => {
      ...
      if (finalUserType === 'CLIENT') {
        ...
        if (!client) {
          ...
          const error = new Error(`Клиент с ID ${finalClientId} не найден`);
          error.status = 400;
          throw error;
        }
