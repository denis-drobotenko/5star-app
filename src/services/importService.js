const { ImportHistory, Company, User, FieldMapping, Client, sequelize } = require('../models');
...
class ImportService {
     ...
     * @returns {Promise<ImportHistory>} - The created ImportHistory record.
    ...
    static async initiateNewImport({ custom_name, client_id, field_mapping_id, user_id }) {
        ...
        try {
            await sequelize.transaction(async (t) => {
                ...
                if (mappingExists.client_id !== parseInt(client_id)) {
                    logger.warn('[ImportService.initiateNewImport] FieldMapping client_id %s does not match provided client_id %s.',
                        mappingExists.client_id, client_id);
            ...
            if (!error.statusCode) {
                logger.error(`[ImportService.initiateNewImport] Database error while creating import history: ${error.message}`, 
    ...
    static async uploadAndProcessImportFile(importId, userId, file) {
            ...
            if (importRecord && importRecord.id) { // Проверяем, что importRecord был загружен
                try {
                    await ImportHistory.update(
                        { status: 'file_upload_failed', status_details: `Ошибка при загрузке файла: ${error.message}` },
        ...
        try {
            ...
            const worksheet = workbook.Sheets[sheetName];
                ...
                const systemFields = await ImportService.getSystemFieldDefinitions(); // Исправлен вызов на статический
            ...
            for (let i = 0; i < dataRows.length; i++) {
                ...
                const rowErrors = [];
                    ...
                    for (const systemFieldName in mappingRules) {
                        ...
                        if (rule.processing_function) {
                            try {
                                ...
                                value = ImportService._applyProcessingFunction(value, rule.processing_function, rule.function_params);
                            ...
                            switch (systemFieldDef.type) {
                                ...
                                case 'INTEGER':
                                    ...
                                    if (!Number.isInteger(Number(cleanedIntValue))) {
                                        ...
                                        formattedValue = parseInt(Number(cleanedIntValue), 10);
                                    }
                                    break;
                                case 'FLOAT':
                                case 'DECIMAL':
                    ...
                    if (identifierFields.length > 0) {
                        ...
                        //    rowErrors.push({ error: `Конфликт идентификаторов: запись с такими данными уже существует (${identifierFields.join(', ')}).`});
                        // }
                    }
        ...
        return {
            ...
            statistics: {
                ...
                rows_successfully_previewed: finalImportRecord.rows_successfully_previewed, // Берем из финальной записи
                rows_failed_preview: finalImportRecord.rows_failed_preview, // Берем из финальной записи
    ...
    static _applyProcessingFunction(value, functionName, params) {
        ...
        switch (functionName) {
            ...
            case 'parseDate':
                ...
                try {
                    ...
                    if (funcParams.format && typeof funcParams.format === 'string' && funcParams.format.trim() !== '') {
                        ...
                        logger.debug(`[ImportService.applyProcFunc][${functionName}] Trying to parse with custom format from params: '${customFormat}' for string: '${originalStr}'`);
                        const parsedDate = moment(originalStr, customFormat, true); 
                        if (parsedDate.isValid()) {
                            const isoDate = parsedDate.toISOString();
                            logger.info(`[ImportService.applyProcFunc][${functionName}] Successfully parsed with custom format '${customFormat}' to ISO: ${isoDate}`);
                            return isoDate;
                } else {
                            logger.warn(`[ImportService.applyProcFunc][${functionName}] Failed to parse with custom format '${customFormat}' for string: '${originalStr}'.`);
                        }
                    }

                    const genericParsedDate = moment(originalStr, moment.ISO_8601, false); 
                    if (genericParsedDate.isValid()) {
                        const isoDate = genericParsedDate.toISOString();
                        logger.info(`[ImportService.applyProcFunc][${functionName}] Successfully parsed with generic moment.ISO_8601 for '${originalStr}' to ISO: ${isoDate}`);
                        return isoDate;
            } else {
                        logger.warn(`[ImportService.applyProcFunc][${functionName}] Generic moment.ISO_8601 failed to parse '${originalStr}'.`);
                    }

                    logger.warn(`[ImportService.applyProcFunc][${functionName}] All parsing attempts failed for value: '${originalStr}'. Returning null.`);
                    return null;
            ...
            default:
                ...
                // Если это объект Date и не был обработан спец. функцией, вернем ISO строку
                if (value instanceof Date) {
                    logger.debug(`[ImportService.applyProcFunc][default] Value is Date, returning ISO string: ${value.toISOString()}`);
                    return value.toISOString();
                }
                // Для остальных типов - исходное значение как строку, если это не null/undefined, иначе null
