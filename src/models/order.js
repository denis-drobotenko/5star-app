...
const Order = sequelize.define('Order', {
  ...
  customer_id: {
    type: DataTypes.STRING, // В schema.sql это varchar, может быть не числом
    allowNull: true,
  },
  telephone: {
    type: DataTypes.STRING,
  ...
  birthday: {
    type: DataTypes.DATEONLY, // Используем DATEONLY для полей типа date
    allowNull: true,
  },
  car_brand: {
    type: DataTypes.STRING,
  ...
  session_id: { // ID импорта, из которого пришли данные
    type: DataTypes.INTEGER,
    allowNull: true,
  },
  client_id: { // Добавляем client_id
    type: DataTypes.INTEGER,
    allowNull: true, // или false, если это обязательное поле
    references: {
      model: 'clients', // Имя таблицы, на которую ссылаемся
      key: 'id',
    },
    // onDelete: 'SET NULL', // или 'CASCADE', в зависимости от требований
    // onUpdate: 'CASCADE',
  },
}, {
  tableName: 'orders',
  timestamps: false, // В schema.sql нет createdAt/updatedAt для orders
});
