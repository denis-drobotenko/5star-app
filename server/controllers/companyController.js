...
async function getAll(req, res, next) {
    ...
    logger.info('[CompanyController.getAll] Request to get all companies. Query params: %o. Requesting user: %o', req.query, req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' });
    try {
...
async function getById(req, res, next) {
    ...
    logger.info('[CompanyController.getById] Request to get company by ID: %s. Requesting user: %o', companyId, req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' });
    try {
...
async function create(req, res, next) {
    ...
    logger.info('[CompanyController.create] Attempting to create company. Body: %o. Requesting user: %o', companyData, req.user ? { id: req.user.id, role: req.user.role } : { id: 'undefined' });
    try {
...
async function update(req, res, next) {
    ...
    logger.info('[CompanyController.update] Attempting to update company ID %s. Body: %o. Requesting user: %o', companyId, updateData, req.user ? { id: req.user.id, role: req.user.role } : { id: 'undefined' });
    try {
...
async function remove(req, res, next) {
    const companyId = req.params.id;
    logger.info('[CompanyController.remove] Attempting to delete company ID %s. Requesting user: %o', companyId, req.user ? { id: req.user.id, role: req.user.role } : { id: 'undefined' });
    try {
