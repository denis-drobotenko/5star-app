...
async function getAll(req, res, next) {
    logger.info('[ClientController.getAll] Request to get all clients. Headers: %o, User: %o', { authorization: req.headers.authorization ? 'present' : 'missing' }, req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' });
    try {
        const clients = await clientService.getAllClients();
        logger.debug('[ClientController.getAll] Found %d clients. First client: %o', clients.length, clients[0] ? { id: clients[0].id, name: clients[0].name } : 'no clients');
        res.json(clients);
    }
    catch (error) {
...
async function getById(req, res, next) {
    ...
    logger.info('[ClientController.getById] Request to get client by ID: %s. Requesting user: %o', clientId, req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' });
    try {
...
async function create(req, res, next) {
    const { name } = req.body;
    const logoFile = req.file; // Типизация req.file
    ...
    logger.info('[ClientController.create] Attempting to create client. Name: %s. File: %o. Requesting user: %o', name, fileInfoLog, req.user ? { id: req.user.id, role: req.user.role } : { id: 'undefined' });
    try {
        ...
        if (error.name === 'SequelizeValidationError') {
            res.status(400).json({ message: 'Ошибка валидации: ' + (error.errors ? error.errors.map((e) => e.message).join(', ') : error.message) });
            return;
        }
...
async function update(req, res, next) {
    ...
    logger.info('[ClientController.update] Attempting to update client ID %s. Name: %s, File: %o, RemoveLogo: %s. Requesting user: %o', id, name, fileInfoLog, removeLogo, req.user ? { id: req.user.id, role: req.user.role } : { id: 'undefined' });
    try {
...
async function remove(req, res, next) {
    const { id } = req.params;
    logger.info('[ClientController.remove] Attempting to delete client ID %s. Requesting user: %o', id, req.user ? { id: req.user.id, role: req.user.role } : { id: 'undefined' });
    try {
