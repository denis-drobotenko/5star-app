...
async function requestPasswordResetHandler(req, res, next) {
    ...
    logger.info('[AuthController.requestPasswordReset] Request received for email: %s. Requesting user (if any): %o', email, req.user ? { id: req.user.id, role: req.user.role } : { id: 'undefined' });
    try {
...
async function resetPasswordHandler(req, res, next) {
    ...
    const { password: _, ...bodyToLog } = req.body;
    logger.info('[AuthController.resetPassword] Attempting to reset password with token (first 8 chars): %s. Body: %o', token ? token.substring(0, 8) : '[NO TOKEN PROVIDED]', bodyToLog);
    try {
        ...
        logger.error('[AuthController.resetPassword] Failed to reset password for token (first 8 chars): %s. Error: %s', token ? token.substring(0, 8) : '[NO TOKEN PROVIDED]', error.message, { requestBody: bodyToLog });
        if (error.status === 400) {
            res.status(400).json({ message: error.message });
            return;
        }
        if (error.name === 'SequelizeValidationError') {
            res.status(400).json({ message: 'Ошибка валидации нового пароля: ' + (error.errors ? error.errors.map((e) => e.message).join(', ') : error.message) });
            return;
        }
        next(error);
    }
