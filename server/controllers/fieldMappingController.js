...
class FieldMappingController {
    ...
    static async getAll(req, res, next) {
        logger.info('[FieldMappingController.getAll] Request to get all mappings. User: %o, Query: %o', req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' }, req.query);
        try {
    ...
    static async getByClientId(req, res, next) {
        ...
        logger.info('[FieldMappingController.getByClientId] Request to get mappings by client ID: %s. Requesting user: %o', clientId, req.user ? { id: req.user.id, role: req.user.role } : { id: 'undefined' });
        try {
            if (req.user && req.user.client_id != null && req.user.client_id !== clientId) {
                logger.warn('[FieldMappingController.getByClientId] Forbidden access for user ID %s (client_id %s) to client ID %s mappings.', req.user.id, req.user.client_id, clientId);
                return res.status(403).json({ message: 'Нет доступа к маппингам другого клиента' });
            }
    ...
    static async getById(req, res, next) {
        ...
        logger.info('[FieldMappingController.getById] Request to get mapping by ID: %s. Requesting user: %o', mappingId, req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' });
        try {
            ...
            if (req.user && req.user.client_id != null && mapping.client_id !== req.user.client_id) {
                logger.warn('[FieldMappingController.getById] Forbidden access for user ID %s (client_id %s) to mapping ID %s (client_id %s).', req.user.id, req.user.client_id, mappingId, mapping.client_id);
    ...
    static async create(req, res, next) {
        ...
        logger.info('[FieldMappingController.create] Attempting to create mapping. Data: %o. Requesting user: %o', mappingDataForLog, req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' });
        ...
        if (req.user && req.user.client_id != null && final_client_id !== req.user.client_id) {
            logger.warn('[FieldMappingController.create] Forbidden: User %s (client_id %s) attempting to create mapping for client_id %s.', req.user.id, req.user.client_id, final_client_id);
    ...
    static async update(req, res, next) {
        ...
        logger.info('[FieldMappingController.update] Attempting to update mapping ID %s. Data: %o. Requesting user: %o', mappingId, updateDataForLog, req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' });
        try {
            ...
            if (userClientId !== null && client_id != null && userClientId !== client_id) {
                logger.warn('[FieldMappingController.update] Forbidden: User %s (client_id %s) attempting to change mapping client_id to %s for mapping ID %s.', req.user.id, userClientId, client_id, mappingId);
                return res.status(403).json({ message: 'Вы не можете изменить ID клиента для этого маппинга.' });
            }
            const updatedMapping = await FieldMappingService.updateMapping(mappingId, { name, mapping, client_id, sample_data_url }, userClientId // client_id текущего пользователя для авторизации в сервисе
            );
            if (!updatedMapping) {
    ...
    static async remove(req, res, next) {
        const mappingId = req.params.id;
        logger.info('[FieldMappingController.remove] Request to delete mapping ID %s. Requesting user: %o', mappingId, req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' });
        try {
            const userClientId = (req.user && req.user.client_id != null) ? req.user.client_id : null;
            const deleted = await FieldMappingService.deleteMapping(mappingId, userClientId);
            if (!deleted) {
                logger.warn('[FieldMappingController.remove] Mapping ID %s not found for deletion (service returned false).', mappingId);
                return res.status(404).json({ message: 'Маппинг не найден' });
            }
            logger.info('[FieldMappingController.remove] Mapping ID %s deleted successfully. Sending 204 response.', mappingId);
            res.status(204).send();
        }
        catch (error) {
            if (error.statusCode === 403) {
                logger.warn(`[FieldMappingController.remove] Forbidden (service error): ${error.message} for mapping ID ${mappingId}`);
                return res.status(403).json({ message: error.message });
            }
            // Не должно быть 404 отсюда, так как сервис вернет false, что обработано выше
            logger.error('[FieldMappingController.remove] Failed to delete mapping ID %s. Error: %s', mappingId, error.message, {
                error: error,
                mappingIdParam: mappingId,
                requestingUser: req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : null
            });
    ...
    static async uploadSample(req, res, next) {
        const mappingId = req.params.id;
        const file = req.file;
        logger.info('[FieldMappingController.uploadSample] Request to upload sample for mapping ID %s. File: %s. User: %o', mappingId, file ? file.originalname : 'No file provided', req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' });
        if (!file) {
    ...
    static async getSignedUrl(req, res, next) {
        ...
        logger.info('[FieldMappingController.getSignedUrl] Request for signed URL. Mapping ID: %s, ExpiresIn: %s, User: %o', mappingId, expiresIn, userForLog);
        try {
            const userClientId = (req.user && req.user.client_id != null) ? req.user.client_id : null;
     ...
     * @returns {Error}  default - Unexpected error (e.g. S3 error).
     */
    static async getS3FileSignedUrl(req, res, _next) {
        const { s3Key } = req.params;
        logger.info('[FieldMappingController.getS3FileSignedUrl] Requesting signed URL for S3 key: %s. User: %o', s3Key, req.user ? { id: req.user.id, role: req.user.role, client_id: req.user.client_id } : { id: 'undefined' });
        if (!s3Key) {
