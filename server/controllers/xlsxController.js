 ...
 */
export async function uploadXLSX(req, res, next) {
    logger.info('[XLSXController.uploadXLSX] Received request to upload XLSX file. File: %s, User: %o', req.file ? req.file.originalname : 'No file', req.user ? { id: req.user.id, role: req.user.role } : { id: 'undefined' });
    try {
        if (!req.file) {
        ...
        logger.error('[XLSXController.uploadXLSX] Error processing XLSX upload. File: %s, Error: %s', req.file ? req.file.originalname : 'N/A', (error instanceof Error ? error.message : 'Unknown error'), {
            error: error,
...
export const processS3XLSX = [
    ...
    async (req, res, next) => {
        ...
        try {
            ...
            logger.error('[XLSXController.processS3XLSX] Error processing S3 XLSX. Key: %s, Error: %s', s3Key, (error instanceof Error ? error.message : 'Unknown error'), {
                error: error,
                s3Key: s3Key,
...
export async function upload(_req, res, _next) {
    // TODO: Реализовать или удалить, если не используется
    logger.warn('[XLSXController.upload] Function not implemented, called for file: %s', _req.file ? _req.file.originalname : 'No file');
    res.status(501).json({ message: 'Not Implemented' });
}
export async function getTemplate(_req, res, _next) {
    // TODO: Реализовать или удалить, если не используется
