...
router.post('/:importId/upload-file', async (req, res, next) => {
    try {
        ...
        if (Array.isArray(importController.uploadAndProcessFile) && importController.uploadAndProcessFile.length > 0) {
            const multerRunner = importController.uploadAndProcessFile[0]; // Первый элемент - multer middleware
            multerRunner(req, res, async (multerError) => {
                if (multerError) {
                ...
                if (importController.uploadAndProcessFile.length > 1) {
                    ...
                    const mainFileHandler = importController.uploadAndProcessFile[1];
