...
import { validateBody, validateParams, validateQuery, mappingIdSchema, clientIdParamsSchema, // Новая схема
createMappingSchema, updateMappingSchema, uploadSampleSchema, // Для /upload-sample
...
router.post('/upload-xlsx', upload.single('file'), (req, res, next) => XLSXController.uploadXLSX(req, res, next));
...
router.post('/process-s3-xlsx', (req, res, next) => {
...
async (req, res, next) => {
    try {
        ...
        await XLSXController.processS3XLSX[1](req, res, next);
    }
    catch (error) {
        next(error);
    }
