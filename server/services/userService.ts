...
async function getAllUsers(filters: any = {}) {
  ...
  try {
    ...
    logger.error('[UserService.getAllUsers] Error fetching users: %s', (error as any).message, { error, filters });
    throw error;
  }
}
...
async function getUserById(userId: any) {
  ...
  try {
    ...
    logger.error('[UserService.getUserById] Error fetching user ID %s: %s', userId, (error as any).message, { error });
    throw error;
  }
}
...
async function createUser(userData: any) {
  ...
  try {
    ...
    if (!(error as any).status) {
        logger.error('[UserService.createUser] Failed to create user. Email: %s. Error: %s', email, (error as any).message, { error, userData });
    }
    throw error;
  }
}

async function updateUser(userId: any, updateData: any) {
  ...
  try {
    await sequelize.transaction(async (t: any) => {
      ...
      const dataToUpdate: any = { ...otherData }; // Начинаем с otherData, чтобы не затереть email/password/client_id/user_type если они там есть и не переданы отдельно
    ...
    if (!(error as any).status) {
        logger.error('[UserService.updateUser] Failed to update user ID %s. Error: %s', userId, (error as any).message, { error, updateData });
    }
    throw error;
  }
}
...
async function deleteUser(userId: any) {
  ...
  try {
    ...
    if (!(error as any).status) {
        logger.error('[UserService.deleteUser] Failed to delete user ID %s. Error: %s', userId, (error as any).message, { error });
    }
    throw error;
  }
}
