...
const transporter = nodemailer.createTransport({
  ...
  port: parseInt(process.env.EMAIL_PORT || '587', 10),    // Порт (587 для TLS/STARTTLS, 465 для SSL)
 ...
 */
async function sendPasswordResetEmail(to: any, token: any) {
  ...
  try {
    logger.info(`[EmailService] Attempting to send password reset email to: ${to}. From: ${mailOptions.from}`);
    ...
    logger.error(`[EmailService] Error sending password reset email to ${to} via SMTP:`, { errorMessage: (error as any).message, errorStack: (error as any).stack, errorFull: JSON.stringify(error, Object.getOwnPropertyNames(error), 2) });
