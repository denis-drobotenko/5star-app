...
async function getAllClients() {
    ...
    try {
        ...
        logger.info(`[ClientService.getAllClients] Successfully fetched ${clients.length} clients. Sample client: %o`, clients[0] ? {
            id: clients[0].id,
            name: clients[0].name,
...
async function createClient(clientData, logoFile) {
    ...
    let logoUrl = null;
    let newClient;
    logger.debug('[ClientService.createClient] Attempting to create client. Name: %s, File provided: %s', name, !!logoFile);
    try {
...
async function updateClient(clientId, updateData, logoFile, removeLogoFlag) {
    ...
    let updatedClient;
    logger.debug('[ClientService.updateClient] Attempting to update client ID: %s. Data: %o, New logo: %s, Remove logo: %s', clientId, updateData, !!logoFile, removeLogoFlag);
    try {
        await sequelize.transaction(async (t) => {
