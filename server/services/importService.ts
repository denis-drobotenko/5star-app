...
class ImportService {
     ...
     */
    static async getAllImportHistories({ clientId, search, sortBy = 'created_at', sortOrder = 'DESC', page = 1, limit = 10 }: any) {
        const offset = (parseInt(page as any, 10) - 1) * parseInt(limit as any, 10);
        const whereClause = {};

        if (clientId) {
            (whereClause as any).client_id = clientId;
        }
        ...
        try {
            ...
            return {
                totalItems: count,
                totalPages: Math.ceil(count / parseInt(limit as any, 10)),
                currentPage: parseInt(page as any, 10),
                items: rows
            };
        } catch (error: any) {
            // Собираем контекст для лога из доступных переменных
            const logContext = { clientId, search, sortBy, sortOrder, page, limit };
            logger.error(`[ImportService.getAllImportHistories] Error getting import histories: ${(error as any).message}`, { error, context: logContext });
            throw error;
        }
    }
     ...
     */
    static async initiateNewImport({ custom_name, client_id, field_mapping_id, user_id }: any): Promise<InstanceType<typeof ImportHistory> | undefined> {
        logger.debug('[ImportService.initiateNewImport] Initiating import for client_id: %s, mapping_id: %s, user_id: %s',
            client_id, field_mapping_id, user_id);
        let newImportRecord: InstanceType<typeof ImportHistory> | undefined;
        ...
        try {
            await sequelize.transaction(async (t: any) => {
                ...
                if (!clientExists) {
                    ...
                    const error: any = new Error("Клиент не найден.");
                ...
                if (!mappingExists) {
                    ...
                    const error: any = new Error("Правило маппинга не найдено.");
                    (error as any).statusCode = 404;
                    throw error;
                }

                if ((mappingExists as any).client_id !== parseInt(client_id)) {
                    logger.warn('[ImportService.initiateNewImport] FieldMapping client_id %s does not match provided client_id %s.',
                        (mappingExists as any).client_id, client_id);
                    const error: any = new Error("Выбранный маппинг не принадлежит указанному клиенту.");
                ...
                logger.info(`[ImportService.initiateNewImport] New import initiated successfully within transaction. Import ID: ${(newImport as any).id}`);
                newImportRecord = newImport; // Присваиваем для возврата
            });
            return newImportRecord;
        } catch (error: any) {
            if (!(error as any).statusCode) {
                logger.error(`[ImportService.initiateNewImport] Database error while creating import history: ${(error as any).message}`, 
                    { error, custom_name, client_id, field_mapping_id, user_id });
            }
            throw error; 
        }
    }
     ...
     */
    static async uploadAndProcessImportFile(importId: any, userId: any, file: any) {
        logger.info('[ImportService.uploadAndProcessImportFile] Uploading and processing file for import ID: %s, User ID: %s, File: %s',
            importId, userId, file.originalname);

        if (!S3_BUCKET_NAME) {
            logger.error('[ImportService.uploadAndProcessImportFile] S3_BUCKET_NAME is not configured.');
            const err: any = new Error("Ошибка сервера: S3_BUCKET_NAME не настроен.");
        ...
        try {
            await sequelize.transaction(async (uploadTransaction: any) => {
                ...
                if (!record) {
                    ...
                    const err: any = new Error("Запись о загрузке не найдена или у вас нет прав на её изменение.");
                ...
                if (!allowedStatusesForUpload.includes(importRecord.status)) {
                    ...
                    const err: any = new Error(`Нельзя загрузить файл для импорта со статусом '${importRecord.status}'.`);
                ...
                if (!client) {
                     ...
                     const err: any = new Error("Клиент, связанный с этой загрузкой, не найден.");
                     (err as any).statusCode = 404; 
                     throw err; 
                }
                ...
                if (oldOriginalFileKey && oldOriginalFileKey !== s3Key) {
                    try {
                        ...
                        logger.warn(`[ImportService.uploadAndProcessImportFile] Failed to delete old original file ${oldOriginalFileKey} from S3: ${(delErr as any).message}`);
                    }
                }
                // Удаляем старые файлы данных предпросмотра, если они были
                    if (oldProcessedDataKey) {
                        try {
                            ...
                            logger.warn(`[ImportService.uploadAndProcessImportFile] Failed to delete old processed data ${oldProcessedDataKey} from S3: ${(delErr as any).message}`);
                        }
                    }
                    if (oldErrorDataKey) {
                        try {
                            ...
                            logger.warn(`[ImportService.uploadAndProcessImportFile] Failed to delete old error data ${oldErrorDataKey} from S3: ${(delErr as any).message}`);
                        }
                    }
            });
        } catch (error: any) {
            logger.error(`[ImportService.uploadAndProcessImportFile] Error during S3 upload or initial DB update for import ID ${importId}: ${(error as any).message}`, { error, importId, userId, fileName: file.originalname });
            // Если файл был загружен на S3, но произошла ошибка при обновлении БД, удаляем файл из S3
            if (s3UploadSuccessful && s3Key && !(error as any).statusCode) { // Предполагаем, что системные ошибки не имеют statusCode
                try {
                    ...
                    logger.error(`[ImportService.uploadAndProcessImportFile] Failed to delete ${s3Key} from S3 during rollback: ${(s3DeleteError as any).message}`, { s3DeleteError });
                }
            }
            ...
            if (importRecord && importRecord.id) { // Проверяем, что importRecord был загружен
                try {
                    await ImportHistory.update(
                        { status: 'file_upload_failed', status_details: `Ошибка при загрузке файла: ${(error as any).message}` },
                        { where: { id: importRecord.id } }
                    );
                } catch (statusUpdateError: any) {
                    logger.error(`[ImportService.uploadAndProcessImportFile] Failed to update import status to 'file_upload_failed' for import ID ${importRecord.id}: ${(statusUpdateError as any).message}`, { statusUpdateError });
                }
            }
        ...
        try {
            await sequelize.transaction(async (processingTransaction: any) => {
                ...
                if (!importRecord) { // Дополнительная проверка, хотя на первом этапе уже есть
                    ...
                    const err: any = new Error("Запись об импорте не найдена перед этапом обработки.");
                    (err as any).statusCode = 404;
                throw err;
            }
                ...
                if (importRecord.status !== 'file_uploaded') {
                    ...
                    const err: any = new Error(`Файл для импорта ${importId} не готов к обработке (статус: ${importRecord.status}).`);
                    (err as any).statusCode = 400;
                throw err;
            }
                ...
                if (!fieldMappingModel || !fieldMappingModel.mapping) { 
                    ...
                    const err: any = new Error("Данные правил маппинга (поле JSONB 'mapping') не найдены или неполны.");
                    (err as any).statusCode = 400; 
                throw err;
            }
                ...
                if (typeof rawMappingObject !== 'object' || rawMappingObject === null || Array.isArray(rawMappingObject)) {
                    ...
                    const err: any = new Error("Формат данных маппинга некорректен (ожидался объект в поле 'mapping').");
                    (err as any).statusCode = 500; 
                    throw err;
                }
                ...
                const fileBuffer: any = await streamToBuffer(s3File.Body);
            ...
            const worksheet = workbook.Sheets[sheetName];
                ...
                if (jsonData.length === 0) {
                    (processingStats.parse_errors as any[]).push("Файл пустой или не удалось прочитать данные.");
                ...
                logger.info(`[ImportService.uploadAndProcessImportFile] File parsed. Headers: [${(headers as any).join(', ')}]. Total data rows: ${processingStats.totalRows}`);
            ...
            for (let i = 0; i < dataRows.length; i++) {
                ...
                const rowErrors: any = [];
                    ...
                    for (const systemFieldName in mappingRules) {
                        ...
                        const headerIndex = (headers as any).indexOf(rule.file_header_name);
                        let value = headerIndex !== -1 ? (row as any)[headerIndex] : null;
                        ...
                        if (rule.processing_function) {
                            try {
                                ...
                                logger.warn(`[ImportService.uploadAndProcessImportFile] Row ${i+1}, Field '${systemFieldName}': Error applying function '${rule.processing_function}': ${(funcError as any).message}`);
                                rowErrors.push({
                                    field: systemFieldName,
                                    header: rule.file_header_name || 'N/A',
                                    value: value,
                                    error: `Ошибка функции '${rule.processing_function}': ${(funcError as any).message}`
                                });
                        ...
                        const systemFieldDef = systemFields.find((sf: any) => sf.name === systemFieldName);
                    ...
                    const identifierFields = Object.entries(mappingRules)
                        .filter(([_, rule]: [string, any]) => (rule as any).is_identifier)
                    ...
                    if (rowErrors.length > 0) {
                        ...
                        (processingStats.errorData as any[]).push({ original_row_index: i + 1, original_data: row, errors: rowErrors, processed_data_attempt: processedRow });
                } else {
                        processingStats.successfullyProcessedRows++;
                        (processingStats.processedData as any[]).push(processedRow);
                        ...
                        if (processingStats.sample_rows.length < 10) { // Ограничение на количество строк в предпросмотре
                           (processingStats.sample_rows as any[]).push(processedRow);
                        }
                    }
                ...
                if (processingStats.failedRows > 0) {
                    ...
                    (processingStats.errorData as any[]).slice(0, 5).forEach((errItem: any) => {
                        logger.info(`  Row index ${(errItem as any).original_row_index}: Errors: ${JSON.stringify((errItem as any).errors)}`);
                    });
                }
            ...
            logger.error(`[ImportService.uploadAndProcessImportFile] Error during file processing or final DB update for import ID ${importId}: ${(processingError as any).message}`, { error: processingError, importId });
            ...
            try {
                ...
                const currentRecord: any = await ImportHistory.findByPk(importId);
                if (currentRecord) {
                    currentRecord.status = 'processing_failed';
                    currentRecord.status_details = `Критическая ошибка при обработке: ${(processingError as any).message.substring(0, 250)}`;
                ...
                logger.error(`[ImportService.uploadAndProcessImportFile] CRITICAL: Failed to update import status to 'processing_failed' for import ID ${importId} after a processing error: ${(statusUpdateError as any).message}`, { statusUpdateError });
            }
            ...
            if (processedDataS3Key && !processingSuccessful) { // Если ключ есть, но основной успех не достигнут
                try {
                    ...
                    logger.error(`[ImportService.uploadAndProcessImportFile] Failed to delete ${processedDataS3Key} from S3 during error rollback: ${(s3DeleteError as any).message}`);
                }
            }
            if (errorDataS3Key && !processingSuccessful) {
                try {
                    ...
                    logger.error(`[ImportService.uploadAndProcessImportFile] Failed to delete ${errorDataS3Key} from S3 during error rollback: ${(s3DeleteError as any).message}`);
                }
            }
        ...
        const finalImportRecord: any = await ImportHistory.findByPk(importId, {
                include: [
                { model: Client, as: 'client' },
                { model: User, as: 'user' },
                { model: FieldMapping, as: 'fieldMapping' }
        ...
        return {
            ...
            statistics: {
                ...
                rows_successfully_previewed: (finalImportRecord as any)?.rows_successfully_previewed,
                rows_failed_preview: (finalImportRecord as any)?.rows_failed_preview,
    ...
    static _applyProcessingFunction(value: any, functionName: any, params: any) {
        ...
        logger.debug(
            `[ImportService.applyProcFunc][${functionName}] Called with value (type ${typeof value}): '${value}', initialStringValue: '${initialStringValue}', params: %o`,
            funcParams
        );
        ...
        switch (functionName) {
            ...
            case 'parseDate':
                ...
                try {
                    ...
                    logger.error(`[ImportService.applyProcFunc][${functionName}] Error during date parsing for value '${initialStringValue}': ${(dateError as any).message}`, { error: dateError });
                    return null;
                }
