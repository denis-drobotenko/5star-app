...
class FieldMappingService {
            ...
            await sequelize.transaction(async (t) => {
                ...
                if (client_id != null && client_id !== existingMapping.client_id) {
                    ...
                    if (!clientExists) {
                        ...
                        const error2 = new Error(`Клиент с ID ${client_id} не найден.`);
    ...
    static async uploadSampleFile(mappingId, file, requesting_client_id) {
        logger.debug('[FieldMappingService.uploadSampleFile] Attempting to upload sample for mapping ID %s. File: %s. Requesting client_id: %s', mappingId, file ? file.originalname : 'No file', requesting_client_id);
        if (!file) {
        ...
        try {
            await sequelize.transaction(async (t) => {
                ...
                // Авторизация
                if (requesting_client_id != null && mapping.client_id !== requesting_client_id) {
                    logger.warn('[FieldMappingService.uploadSampleFile] Forbidden: User with client_id %s cannot upload sample for mapping ID %s (client_id %s).', requesting_client_id, mappingId, mapping.client_id);
                    const error2 = new Error('Нет доступа к этому маппингу для загрузки образца');
                    error2.statusCode = 403;
                    throw error2;
                }
                ...
                logger.info('[FieldMappingService.uploadSampleFile] File %s uploaded to S3 for mapping ID %s. S3 Key: %s', file.originalname, mappingId, s3Key);
            ...
            logger.error('[FieldMappingService.uploadSampleFile] Error during upload/DB update for mapping ID %s. Error: %s', mappingId, error.message, { error, s3KeyIfGenerated: s3Key, fileName: file.originalname, s3UploadSuccessful });
            ...
            if (s3UploadSuccessful) {
                ...
                try {
                    ...
                    logger.error('[FieldMappingService.uploadSampleFile] CRITICAL: Failed to delete S3 file %s after DB error. Manual cleanup required. S3 Delete Error: %s', s3Key, s3DeleteError.message, { error: s3DeleteError });
    ...
    static async getSampleFileSignedUrl({ mappingId, s3Key, requesting_client_id, expiresIn = 3600 }) {
        logger.debug('[FieldMappingService.getSampleFileSignedUrl] Request for signed URL. Mapping ID: %s, S3 Key: %s, ExpiresIn: %s, Requesting client_id: %s', mappingId, s3Key, expiresIn, requesting_client_id);
        ...
        if (!targetS3Key && mappingId) {
            ...
            if (requesting_client_id != null && mapping.client_id !== requesting_client_id) {
                logger.warn('[FieldMappingService.getSampleFileSignedUrl] Forbidden: User with client_id %s cannot access sample for mapping ID %s (client_id %s).', requesting_client_id, mappingId, mapping.client_id);
        ...
        try {
            ...
            logger.info('[FieldMappingService.getSampleFileSignedUrl] Signed URL generated for S3 key %s (Mapping ID: %s).', targetS3Key, mappingId || 'N/A (direct key)');
            return signedUrl;
        }
        catch (error) {
            logger.error('[FieldMappingService.getSampleFileSignedUrl] S3 error for key %s (Mapping ID: %s). Error: %s', targetS3Key, mappingId || 'N/A (direct key)', error.message, { error });
            throw error;
        }
    }
}
