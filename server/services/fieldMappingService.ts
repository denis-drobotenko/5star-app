...
import { FieldMapping, Client, sequelize } from '../../shared/models/index.js';
...
class FieldMappingService {
     ...
     */
    static async getAllMappings(client_id: any) {
        ...
        try {
            ...
            logger.error('[FieldMappingService.getAllMappings] Failed to get mappings. Error: %s', (error as any).message, { error });
            throw error;
        }
    }
     ...
     * @throws {Error} If there's an error during the database query.
     */
    static async getMappingsByClientId(clientId: any) {
        ...
        try {
            ...
            logger.error('[FieldMappingService.getMappingsByClientId] Failed to get mappings for client ID %s. Error: %s', clientId, (error as any).message, { error });
            throw error;
        }
    }
     ...
     * @throws {Error} If there's an error during the database query.
     */
    static async getMappingById(mappingId: any) {
        ...
        try {
            ...
            logger.error('[FieldMappingService.getMappingById] Failed to get mapping by ID %s. Error: %s', mappingId, (error as any).message, { error });
            throw error;
        }
    }
     ...
     */
    static async createMapping({ name, mapping, client_id, sample_data_url }: any) {
        ...
        try {
            await sequelize.transaction(async (t: any) => {
                if (client_id) {
                    ...
                    if (!clientExists) {
                        ...
                        const error: any = new Error(`Клиент с ID ${client_id} не найден.`);
            ...
            logger.error('[FieldMappingService.createMapping] Failed to create mapping. Error: %s', (error as any).message, {
                error: error,
     ...
     */
    static async updateMapping(mappingId: any, { name, mapping, client_id, sample_data_url }: any, requesting_client_id: any) {
        ...
        try {
            await sequelize.transaction(async (t: any) => {
                ...
                if (!existingMapping) {
                    ...
                    const notFoundError: any = new Error('Маппинг не найден');
                ...
                if (requesting_client_id != null && existingMapping.client_id !== requesting_client_id) {
                    ...
                    const error: any = new Error('Нет доступа к обновлению этого маппинга');
                ...
                if (client_id != null && client_id !== existingMapping.client_id) { 
                    ...
                    if (!clientExists) {
                        ...
                        const error2: any = new Error(`Клиент с ID ${client_id} не найден.`);
                        (error2 as any).statusCode = 400;
                        throw error2;
                    }
            ...
            if (!(error as any).statusCode) {
                logger.error('[FieldMappingService.updateMapping] Failed to update mapping ID %s. Error: %s', mappingId, (error as any).message, { error, updateDataForLog });
            }
            throw error;
        }
    }
     ...
     */
    static async deleteMapping(mappingId: any, requesting_client_id: any) {
        ...
        try {
            await sequelize.transaction(async (t: any) => {
                ...
                if (requesting_client_id != null && mapping.client_id !== requesting_client_id) {
                    ...
                    const error: any = new Error('Нет доступа к удалению этого маппинга');
                    (error as any).statusCode = 403;
                    throw error;
                }
            ...
            if (!(error as any).statusCode) { // Если это не наша кастомная ошибка со статусом
                 logger.error('[FieldMappingService.deleteMapping] Failed to delete mapping ID %s. Error: %s', mappingId, (error as any).message, { error });
            }
            throw error;
        }
    }
     ...
     */
    static async uploadSampleFile(mappingId: any, file: any, requesting_client_id: any) {
        ...
        if (!file) {
            ...
            const error: any = new Error('Файл не предоставлен.');
            (error as any).statusCode = 400;
            throw error;
        }
        ...
        try {
            await sequelize.transaction(async (t: any) => {
                ...
                if (requesting_client_id != null && mapping.client_id !== requesting_client_id) {
                    ...
                    const error2: any = new Error('Нет доступа к этому маппингу для загрузки образца');
                    (error2 as any).statusCode = 403;
                    throw error2;
                }
            ...
            logger.error('[FieldMappingService.uploadSampleFile] Error during upload/DB update for mapping ID %s. Error: %s', 
                mappingId, (error as any).message, { error, s3KeyIfGenerated: s3Key, fileName: file.originalname, s3UploadSuccessful });
            ...
            if (s3UploadSuccessful) {
                ...
                try {
                    ...
                    logger.error('[FieldMappingService.uploadSampleFile] CRITICAL: Failed to delete S3 file %s after DB error. Manual cleanup required. S3 Delete Error: %s',
                        s3Key, (s3DeleteError as any).message, { error: s3DeleteError });
     ...
     */
    static async getSampleFileSignedUrl({ mappingId, s3Key, requesting_client_id, expiresIn = 3600 }: any) {
        ...
        if (!targetS3Key && mappingId) {
            ...
            if (requesting_client_id != null && mapping.client_id !== requesting_client_id) {
                logger.warn('[FieldMappingService.getSampleFileSignedUrl] Forbidden: User with client_id %s cannot access sample for mapping ID %s (client_id %s).',
                    requesting_client_id, mappingId, mapping.client_id);
                const error: any = new Error('Нет доступа к образцу данных этого маппинга');
                (error as any).statusCode = 403;
                throw error;
            }
